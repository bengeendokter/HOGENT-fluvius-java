...
public class AanmeldPogingDaoJpa extends GenericDaoJpa<AanmeldPoging> implements AanmeldPogingDao
{
	...
	{
		return em.createNamedQuery("aanmeldpoging.findByGebruiker", AanmeldPoging.class)
				.setParameter("gebruiker", gebruiker).getResultList().stream().findFirst().orElse(null);//
	}
}
